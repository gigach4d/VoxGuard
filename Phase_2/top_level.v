...
module top_level (
    ...
    wire rst = rst_n; // Active-High Reset on Cmod S7
    ...
    // 2. Transmit & Receive FIFOs
    fwft_fifo #(.DEPTH(256)) tx_fifo (
        .clk(clk), .rst(rst),
        .wr_en(adc_valid && !tx_fifo_full && push_to_talk), .din(adc_data),
    ...
    fwft_fifo #(.DEPTH(256)) rx_fifo (
        .clk(clk), .rst(rst),
        .wr_en(rx_fifo_wr_en), .din(rx_fifo_din),
        .rd_en(dac_ready && !rx_fifo_empty), .dout(dac_data),
        .empty(rx_fifo_empty), .full(rx_fifo_full)
    );

    // 3. Logic & Encryption Glue
    ...
    // 4. The Standard Radio Core
    nrf24_core radio (
        .clk(clk), .rst(rst), .tx_en(push_to_talk),
        .start_tx(start_tx), .tx_data(tx_pkt), .tx_done(tx_done),
        .rx_ready(rx_ready), .rx_data(rx_pkt),
        .ce(radio_ce), .csn(radio_csn), .sck(radio_sck), .mosi(radio_mosi),
        .miso(radio_miso), .irq(radio_irq)
    );

    assign led[0] = push_to_talk;
    assign led[1] = !tx_fifo_empty;
    assign led[2] = !rx_fifo_empty;
    assign led[3] = !radio_irq; // Blinks on TX/RX
endmodule